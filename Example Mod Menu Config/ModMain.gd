...
const MOD_NAME = "Mod Menu Example Mod"
...
func _init(modLoader = ModLoader):
	...
	# Loading files based on whether the config is true or not.
	# This example will load the two files if addEquipment is true, but addEvents is not
	if modSettings["mainToggles"]["addEquipment"] and not modSettings["mainToggles"]["addEvents"]:
		replaceScene("Example/Scene.tscn")
		installScriptExtension("Example/Script.gd")
...
func _ready():
	l("Readying")
	# The translations are updated on ready to ensure the mod menu needed translations exist.
...
func loadSettings():
	...
	settings.loadExample_Mod_Menu_ConfigFromFile()
	settings.saveExample_Mod_Menu_ConfigToFile()
	modSettings = settings.Example_Mod_Menu_ConfigConfig
